...
const JOURNAL_ARTICLES = journalData as JournalArticle[];
      ...
      <div className="max-w-[1800px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 pb-8 border-b border-[#D6D1C7]">
          <div>
            ...
            <span className="block text-xl font-light text-[#A8A29E] mt-2">設計日誌</span>
          </div>
        </div>
          ...
          {JOURNAL_ARTICLES.map((article) => (
            <div key={article.id} className="group cursor-pointer flex flex-col text-left" onClick={() => onArticleClick(article)}>
              ...
              <div className="flex flex-col flex-1 text-left">
                ...
                <h3 className="text-2xl font-serif text-[#2C2A26] mb-1 leading-tight group-hover:underline decoration-1 underline-offset-4">{article.title}</h3>
                <h4 className="text-lg font-light text-[#5D5A53] mb-4">{article.titleZh}</h4>
                <p className="text-[#5D5A53] font-light leading-relaxed mb-2">{article.excerpt}</p>
                <p className="text-[#A8A29E] font-light text-sm leading-relaxed">{article.excerptZh}</p>
              </div>
            </div>
          ))}
        </div>
